...
{//继承
				...
				{//如果文件名存在了
					...
					if (line != null) 
					{
						...
						{//密码正确
							...
							for (ServerThread otherClient : connectedClients) 
							{//相当于两段消息
								otherClient.getDos().writeInt(10);
							...
							for (ServerThread otherClient : connectedClients) 
							{
								...
								{
									otherClient.getDos().writeInt(12);
									String namelist = "";
	...
	void readFileMsg(String msg_name, ServerThread otherClient) 
	{//最好在读取离线文件后将离线文件删除
		try 
		{
	...
	void userRegister() 
	{
		try 
		{
			...
			{
				String name = System.getProperty("user.dir") + "/" + line_array[0] + ".mima";
				String answer = System.getProperty("user.dir") + "/" + line_array[0] + ".ans";
				if (!new File(name).exists()) 
				{//该文件不存在
					userNames[server.allNum++] = line_array[0];//在列表中添加用户名
					allList.repaint();
...
//					}
//					output.write(Integer.parseInt(newPassword));// 将密码按字节变为48进制
					output.write((new BigInteger(line_array[1].getBytes()).add(mypass)).toString(48).getBytes());// 密码加密
					outputanswer.write((new BigInteger(line_array[2].getBytes()).add(mypass)).toString(48).getBytes());// 密码加密
					output.close();
					outputanswer.close();
	...
	void passGetBackLogin() 
	{
		try 
		{
			...
			{
				String name = System.getProperty("user.dir") + "/" + read_array[0] + ".mima";//密码文件
				String answer = System.getProperty("user.dir") + "/" + read_array[0] + ".ans";//密保问题文件
				if(new File(answer).exists()) 
				{//密保文件存在
					String passanswer = (new BigInteger(read_array[2].getBytes()).add(mypass)).toString(48);//密保问题答案
					...
					{//密码正确
						...
						{//名字存在就处理，不存在不处理
							OutputStream output = new FileOutputStream(name);
							output.write((new BigInteger(read_array[1].getBytes()).add(mypass)).toString(48).getBytes());// 密码加密
							output.close();
							for (ServerThread otherClient : connectedClients) 
							{
								if (otherClient.equals(this)) 
